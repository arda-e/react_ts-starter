...
describe('useApi', () => {
  ...
  test('triggers only when callback is called', async () => {
    ...
    const { result, waitForNextUpdate } = renderHook(() => useApi(mockService.mockGetServiceMethod))
  ...
  test('sets error if network error occurs', async () => {
    ...
    expect(apiResponse.error).toEqual(Error('Network Error'))
  })
  ...
  test('sets error if backend error occurs', async () => {
    ...
    expect(apiResponse.error).toEqual(Error('Request failed with status code 500'))
  })
